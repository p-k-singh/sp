...
const Assignment = (props) => {
    ...
    useEffect(()=>{
        ...
        for(var i=0;i<trucks.length;i++){
            if(trucks[i].details!==null)
            sum+=Number(trucks[i].details.capacity)
        }
    ...
    function loadData(){
        ...
        .then((currentUser)=>{
            ...
            API
            .get("GoFlexeOrderPlacement", `/capacity?type=owner&ownerId=${owner}&asset=truck`)
            .then(resp=>{
                console.log(resp)
                ...
                for(var i =0;i<resp.length;i++){
                    ...
                    temp.push({
                        ...
                        capacity:resp[i].capacity===undefined?0:Number(resp[i].capacity),
    ...
    <div style={ i%2===1?{backgroundColor:'#f9f9fb'}:{backgroundColor:'#fff'}}>
        ...
        <Grid  container spacing={3} >
            ...
            <Grid  item xs={12} sm={4} >
                ...
                id={`combo-box-demo${i}`}
                options={myTrucks}
                getOptionLabel={(option) => option.truckNumber+`(${option.capacity}tons)`}
        ...
        <Grid  item xs={12} sm={2} >
                ...
                onChange={(event,value,reason)=>onDriverChanged(event,value,reason,i)}
                getOptionSelected={(option, value) => option.name === value.name }

                style={{ width: 300 }}
                renderInput={(params) => <TextField {...params} label="Driver Name" variant="outlined" />}
                />
        </Grid>
        <Grid  item xs={12} sm={4} >
        <TextField fullWidth
         id="outlined-basic" label="Phone" 
         value={drivers[i].phone} variant="outlined"
                    ...
                    <Card className={classes.root}>  
                            <CardContent style={{ padding: 0 }}>
                                ...
                                <form>
                                    ...
                                    <Grid container  style={{fontFamily:'sans-serif',fontSize:16, backgroundColor:'#f6f4f4' ,padding: 50, paddingTop: 10, paddingBottom: 10 }}>
                                    <Grid item xs={12} sm={4}>
                                      Total Capacity Required : {capacityRequired} tons
                                    </Grid>
                                    <Grid
                                        style={capacityRequired>capacityAlloted?{color:'red'}:(capacityRequired===capacityAlloted)?{color:'green'}:{color:'#f9a825'}}
                                    item xs={12} sm={4}>
                                        Capacity Alloted: {capacityAlloted}tons
                                    </Grid>
                                    <Grid item xs={12} sm={4}>
                                        Capacity Left: {Math.max(capacityRequired-capacityAlloted,0).toFixed(3)}tons
                                    </Grid>
                                    </Grid>
                                    <Button
                                    style={{backgroundColor:'#f9a825',marginTop:10,marginLeft:10}}
                                    onClick={()=>addTruck()}
                                    >
                                    Add Truck
                                </Button>
