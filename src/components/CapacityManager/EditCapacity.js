...
const EditCapacity = (props) => {
  const classes = useStyles();

  const [type, setType] = useState(
    constants.CapacityTypeMap[props.row.assetType]
  );
  const [currentUser, setCurrentUser] = useState(null);
  const [truckNumber, setTruckNumber] = useState(props.row.assetNumber);
  const [CostId, setCostId] = useState(props.row.costId);
  ...
  const [availableFrom, setAvailableFrom] = useState(
    props.row.availableFromDateTime
  );
  ...
  function setChangesBasedOnCapability(capability) {
    ...
    for (var i = 0; i < costData.length; i++) {
      if (!capability || capability.value !== costData[i].capability) continue;
      tempCost = {
        capability: capability,
  ...
  function setChangesBasedOnCapabilityAndCapacity(capability, capacity) {
    ...
    for (var i = 0; i < costData.length; i++) {
      if (!capability || capability.value !== costData[i].capability) continue;
      if (!capacity || capacity.value !== costData[i].capacity) continue;
      tempCost = {
        capability: capability,
        capacity: capacity,
        distance: getDistance(costData[i].rangeinkms),
  ...
  function setChangesBasedOnCapabilityAndDistance(capability, distance) {
    ...
    for (var i = 0; i < costData.length; i++) {
      ...
      if (
        !distance ||
        distance.value.lowRange !== costData[i].rangeinkms.lowRange ||
        distance.value.highRange !== costData[i].rangeinkms.highRange
      )
        continue;
  ...
  function setChangesBasedOnCapabilityAndDistanceAndCapacity(
    capability,
    capacity,
    distance
  ) {
  ...
  const getDistance = (distance) => {
    for (var i = 0; i < constants.DistanceOptions.length; i++) {
      if (
        distance.lowRange === constants.DistanceOptions[i].value.lowRange &&
        distance.highRange === constants.DistanceOptions[i].value.highRange
      ) {
  ...
  const basicCostDetailsChangeController = (event, field) => {
    ...
    if (field === "capability") {
      ...
      result = setChangesBasedOnCapabilityAndDistanceAndCapacity(
        basicCostDetails.capability,
        event,
        basicCostDetails.distance
      );

      if (!result.tempAdditional) {
        result = setChangesBasedOnCapabilityAndCapacity(
          basicCostDetails.capability,
          event
        );
      }
      ...
      result = setChangesBasedOnCapabilityAndDistanceAndCapacity(
        basicCostDetails.capability,
        basicCostDetails.capacity,
        event
      );
      if (!result.tempAdditional)
        result = setChangesBasedOnCapabilityAndDistance(
          basicCostDetails.capability,
          event
        );
      if (!result.tempAdditional) {
  ...
  useEffect(async () => {
    ...
    await API.get(
      ...
      .catch((error) => {
       ...
       await setBasicCostDetails({
         capability: constants.truckCapabilityMap[props.row.capability],
       });
      await setChangesBasedOnCapability(
    ...
    for (var i = 0; i < costData.length; i++) {
      if (basicCostDetails.capability.value === costData[i].capability) {
        if (basicCostDetails.capacity.value === costData[i].capacity) {
          if (
            basicCostDetails.distance.value.lowRange ===
              costData[i].rangeinkms.lowRange &&
            basicCostDetails.distance.value.highRange ===
              costData[i].rangeinkms.highRange
          ) {
